...
export async function getHealth() {
  const result = await query(`select
    ...
    (select count(*)::int from public.punk_saas_mcp_servers) as mcp_servers`);
  return {
    ok: true,
    app: APP.name,
    version: APP.version,
...
export async function getDashboardData() {
  const [stats, agents, board, messages, records, clients, mcpServers] = await Promise.all([
    query(`select
      (select count(*)::int from public.punk_records) as records,
      (select count(*)::int from public.punk_chunks) as chunks,
    ...
    query(`select id, name, slug, description, category, transport, url, command, args, env_template, headers_template, status, official, metadata, last_checked_at, created_at
      from public.punk_saas_mcp_servers
      order by official desc, status='enabled' desc, category asc, name asc`)
  ]);
  return {
    app: APP,
    stats: stats.rows[0],
