...
export async function getDashboardData() {
  const [stats, agents, board, messages, records] = await Promise.all([
    query(`select
    ...
    query(`select
      ...
      join public.punk_saas_boards b on b.id = c.board_id and b.slug='command-board'
    ...
    query(`select m.id, m.author_name, m.body, m.kind, m.created_at, a.role
      ...
      join public.punk_saas_chat_rooms r on r.id=m.room_id and r.slug='war-room'
